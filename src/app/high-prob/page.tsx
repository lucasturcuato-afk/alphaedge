"use client";
// src/app/high-prob/page.tsx
// HIGHEST PROBABILITY BETS — sorted by model win probability, not edge
...
import { TrendingUp, Shield, Star, ChevronRight, AlertTriangle, Flame } from "lucide-react";

const PROP_PICKS = [...MOCK_PROPS]
  .map((p) => ({
    prop: p,
    hitProb: Math.max(p.overProbability, p.underProbability),
    side: p.overProbability >= p.underProbability ? "OVER" : "UNDER",
  }))
  .filter((p) => p.hitProb >= 0.55)
  .sort((a, b) => b.hitProb - a.hitProb)
  .slice(0, 10);

const GAME_PICKS = MOCK_GAMES
  ...
  .map((g) => {
    ...
    return {
      game: g, pred,
      winProb: homeWins ? pred.winProbHome : pred.winProbAway,
      favTeam: homeWins ? g.homeTeam : g.awayTeam,
      undTeam: homeWins ? g.awayTeam : g.homeTeam,
      favSpread: homeWins ? g.lines?.[0]?.homeSpread : g.lines?.[0]?.awaySpread,
      favML: homeWins ? g.lines?.[0]?.homeML : g.lines?.[0]?.awayML,
    };
  })
  .sort((a, b) => b.winProb - a.winProb);

const TOTAL_PLAYS = MOCK_GAMES
  .filter((g) => g.prediction && g.lines?.[0])
  .map((g) => {
    const diff = g.prediction!.projectedTotal - g.lines![0].total;
    return { game: g, diff, side: diff < 0 ? "UNDER" : "OVER", absDiff: Math.abs(diff) };
  })
  .filter((t) => t.absDiff >= 2.5)
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-text-muted ml-10">
            Bets the model believes will <span className="text-accent-green font-semibold">hit</span> — ranked by win probability, not just value
          </p>
        </div>
        <div className="text-right">
          <div className="text-[9px] font-mono text-text-muted">MODEL v2.1 · 10K SIMS</div>
          <div className="text-[9px] font-mono text-text-muted">Thursday, March 26, 2026</div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-2 p-3 bg-accent-amber/5 border border-accent-amber/20 rounded-lg">
        ...
        <p className="text-[10px] text-accent-amber font-mono leading-relaxed">
          <strong>MODEL NOTE:</strong> Win probability = model's estimated chance this bet hits based on stats, matchups, injuries, and pace data.
          A 75% play still loses 1-in-4 times. Never risk more than you can afford to lose.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="relative p-4 rounded-xl border-2 border-accent-green/50 bg-accent-green/5">
            ...
            <div className="mt-1 space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-[10px] text-text-muted font-mono">
                    {PROP_PICKS[0].prop.propType.replace(/_/g,' ').toUpperCase()} {PROP_PICKS[0].side} {PROP_PICKS[0].prop.line}
                  </div>
                </div>
              ...
              <div className="text-[10px] font-mono text-text-muted">
                Proj <span className="text-accent-green font-semibold">{PROP_PICKS[0].prop.projectedValue}</span>
                {" vs line "}<span className="text-text-secondary">{PROP_PICKS[0].prop.line}</span>
                {" · Edge "}<span className="text-accent-green font-semibold">+{PROP_PICKS[0].prop.edge}%</span>
              </div>
              {PROP_PICKS[0].prop.factors[0] && (
                <div className="p-2 bg-bg-secondary rounded text-[9px] text-text-muted leading-relaxed">
                  <span className="text-accent-green font-semibold">{PROP_PICKS[0].prop.factors[0].label}:</span>{" "}
                  {PROP_PICKS[0].prop.factors[0].description}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="relative p-4 rounded-xl border-2 border-accent-blue/50 bg-accent-blue/5">
            ...
            <div className="mt-1 space-y-2">
              ...
              <div className="text-[10px] font-mono text-text-muted">
                Win prob <span className="text-accent-blue font-semibold">{Math.round(GAME_PICKS[0].winProb * 100)}%</span>
                {GAME_PICKS[0].favML && <>{" · ML "}<span className="text-text-secondary">{GAME_PICKS[0].favML > 0 ? "+" : ""}{GAME_PICKS[0].favML}</span></>}
              </div>
              {GAME_PICKS[0].pred.supportingFactors[0] && (
                <div className="p-2 bg-bg-secondary rounded text-[9px] text-text-muted leading-relaxed">
                  <span className="text-accent-blue font-semibold">{GAME_PICKS[0].pred.supportingFactors[0].label}:</span>{" "}
                  {GAME_PICKS[0].pred.supportingFactors[0].description}
                </div>
              )}
            </div>
          </div>
      ...
      {/* FULL PROP PROBABILITY RANKINGS */}
      <div>
            ...
            <Link key={pick.prop.id} href={`/player/${pick.prop.player.id}`}>
              <div className="flex items-center gap-3 p-3 bg-bg-card border border-bg-border rounded-lg hover:border-accent-green/30 transition-colors group">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-mono font-bold shrink-0 ${
                  i===0?"bg-accent-green text-bg-primary":i===1?"bg-accent-amber/20 text-accent-amber border border-accent-amber/30":i===2?"bg-accent-blue/20 text-accent-blue border border-accent-blue/30":"bg-bg-secondary text-text-muted"
                }`}>{i+1}</div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {pick.prop.factors[0] && (
                    <div className="text-[9px] text-text-muted mt-1.5 leading-relaxed">
                      <span className="text-text-secondary font-medium">{pick.prop.factors[0].label}:</span>{" "}{pick.prop.factors[0].description}
                    </div>
                  )}
                </div>
            ...
            <Link key={pick.game.id} href={`/game/${pick.game.id}`}>
              <div className="p-3 bg-bg-card border border-bg-border rounded-lg hover:border-accent-blue/30 transition-colors group">
                <div className="flex items-center gap-3">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-mono font-bold shrink-0 ${
                    i===0?"bg-accent-blue text-bg-primary":i===1?"bg-accent-blue/20 text-accent-blue border border-accent-blue/30":"bg-bg-secondary text-text-muted"
                  }`}>{i+1}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1.5">
                      <SportBadge sport={pick.game.sport} />
                      <span className="text-sm font-semibold text-text-primary">
                        {pick.favTeam.shortName} <span className="text-text-muted font-normal text-xs">vs {pick.undTeam.abbreviation}</span>
                      </span>
                      <ConfBadge prob={pick.winProb} />
                    </div>
                    <ProbBar prob={pick.winProb} />
                    {pick.pred.supportingFactors[0] && (
                      <div className="text-[9px] text-text-muted mt-1.5 leading-relaxed">
                        <span className="text-text-secondary">{pick.pred.supportingFactors[0].label}:</span>{" "}
                        {pick.pred.supportingFactors[0].description}
                      </div>
                    )}
                  </div>
                  <div className="text-right shrink-0 hidden sm:block">
                    ...
                    <div className="text-xs font-mono font-semibold text-text-primary">
                      {pick.favML?`${pick.favML>0?"+":""}${pick.favML}`:"—"}{" / "}{pick.favSpread?`${pick.favSpread>0?"+":""}${pick.favSpread}`:"—"}
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {TOTAL_PLAYS.map((t) => (
            <Link key={t.game.id} href={`/game/${t.game.id}`}>
              <div className="p-3 bg-bg-card border border-bg-border rounded-lg hover:border-accent-amber/30 transition-colors group">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <SportBadge sport={t.game.sport} />
                    <span className="text-xs font-semibold text-text-primary">{t.game.awayTeam.abbreviation} @ {t.game.homeTeam.abbreviation}</span>
                  </div>
                  <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded ${
                    t.side==="UNDER"?"bg-accent-red/15 text-accent-red border border-accent-red/25":"bg-accent-green/15 text-accent-green border border-accent-green/25"
                  }`}>{t.side} {t.game.lines?.[0]?.total}</span>
                </div>
                <div className="flex items-center justify-between text-[10px] font-mono">
                  <span className="text-text-muted">
                    Book <span className="text-text-secondary">{t.game.lines?.[0]?.total}</span>
                    {" → "}Model <span className={t.side==="UNDER"?"text-accent-red font-semibold":"text-accent-green font-semibold"}>
                      {t.game.prediction?.projectedTotal?.toFixed(1)}
                    </span>
                  </span>
                  <span className={`font-bold text-xs ${t.side==="UNDER"?"text-accent-red":"text-accent-green"}`}>
                    {t.diff > 0 ? "+" : ""}{t.diff.toFixed(1)} pts
                  </span>
                </div>
                {t.game.prediction?.supportingFactors?.[0] && (
                  <div className="text-[9px] text-text-muted mt-1.5 leading-relaxed">
                    {t.game.prediction?.supportingFactors?.[0]?.description}
                  </div>
                )}
              </div>
            </Link>
